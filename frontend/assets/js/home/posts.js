    ...
    .then(data => {
        ...
        for(let i = 0; i < data.length; i++){
            ...
            if(isAdmin === 1 || userId === data[i].userId){
                articlePost += '<span class="edit"><i class="fas fa-pencil-alt" onclick="editPost('+ data[i].id + ', '+ data[i].userId + ')"></i>';
                articlePost += '<i class="fas fa-trash" onclick="deletePost('+ data[i].id + ', '+ data[i].userId + ')"></i></span>';
                articlePost += '</div><div class="edit" id="post'+ data[i].id + '"><textarea id="post'+ data[i].id + '"></textarea>';
                articlePost += '<div class="button b-red white" id="post'+ data[i].id + '">Modifier</div></div>'
            } else {
                articlePost += '</div>';
            }
