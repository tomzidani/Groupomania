...
exports.addPost = (req, res, next) => {
    ...
    database.query('SELECT id, name, surname FROM users WHERE id = ? LIMIT 1', [userId])
        .then(data => {
...
exports.getLastsPosts = (req, res, next) => {
    database.query('SELECT * FROM posts ORDER BY -id LIMIT 30')
...
exports.editPost = (req, res, next) => {
    ...
    database.query('SELECT id, admin FROM users WHERE id = ?', [userId])
        .then(data => {
            ...
            if(userId !== req.body.userPostId && data.admin !== 1) return res.status(401).json({ error: 'Unauthorized' });

            // Mise à jour des informations du post dans la base de données
            database.query('UPDATE posts SET message = ? WHERE id = ?', [req.body.message, id])
